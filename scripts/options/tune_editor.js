import { createBooper, createTunePlayer } from '../global/tune_player.js';
...
document.querySelector('.pitch-template > .pitch-slider').max =
...
var flashName = function (index, duration) {
	...
	pitchName.style.borderColor =
		availableColors[availablePitches.indexOf(tune[index])];
...
var playTune = function () {
	// Get townTuneVolume from chrome.storage.local
	chrome.storage.local.get(['townTuneVolume'], (result) => {
		var townTuneVolume =
			result.townTuneVolume !== undefined
				? result.townTuneVolume
				: defaultTownTuneVolume;

		disableEditor();
		tunePlayer
			.playTune(tune, booper, 240, townTuneVolume)
			.eachNote(flashName)
			.done(enableEditor);
	});
};
...
var randomizeTune = function () {
	...
	for (let i = 0; i < tuneLength; i++) {
		...
		let pitch =
			availablePitches[
				2 + Math.floor(Math.random() * availablePitches.length - 2)
			];
		tune[i] = pitch;
