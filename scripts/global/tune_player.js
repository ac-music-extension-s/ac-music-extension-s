...
export var createBooper = function (audioContext) {
	...
	var pitchToFreq = function (pitch) {
		...
		if (freq == 'random')
			freq =
				frequencies[2 + Math.floor(Math.random() * (frequencies.length - 3))];
	...
	var playNote = function (pitch, time, sustainDuration, volume) {
		...
		gain.gain.setValueAtTime(
			sustainLevel * volume,
			time + attack + decay + sustainDuration
		);
		gain.gain.linearRampToValueAtTime(
			0,
			time + attack + decay + sustainDuration + release
		);
	};
