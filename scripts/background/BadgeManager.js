...
export function BadgeManager(addEventListener, isEnabledStart) {
	...
	function processEvent(type, args, useServiceWorkerIconPaths = false) {
		if (type === 'hourMusic') {
			...
			if (lastIconType === 'kk') {
				printDebug(
					'[BadgeManager] Transitioning from KK mode back to hourly music'
				);
			}
			...
			if (useServiceWorkerIconPaths) {
				...
				actionApi
					.setIcon({ path: getIconPaths(weather) })
					.then(() => {
						printDebug(
							'[BadgeManager] Weather icon set successfully to:',
							weather
						);
					})
					.catch((error) => {
			...
			if (useServiceWorkerIconPaths) {
				...
				actionApi
					.setIcon({ path: getIconPaths('paused') })
					.then(() => {
			...
			if (useServiceWorkerIconPaths) {
				...
				actionApi
					.setIcon({ path: getIconPaths(iconToRestore) })
					.then(() => {
						printDebug(
							`[BadgeManager] ${lastIconType === 'kk' ? 'KK' : 'Weather'} icon restored successfully to:`,
							iconToRestore
						);
					})
					.catch((error) => {
				...
				if (lastIconType === 'kk') {
					...
					setIcon(lastWeather);
					printDebug(
						'[BadgeManager] unpause event, set weather icon:',
						lastWeather
					);
				}
			}
			...
			if (lastIconType === 'kk') {
				lastWeather = weather;
				printDebug(
					'[BadgeManager] gameChange/weatherChange during KK mode - updating weather but keeping KK icon'
				);
				return;
			}
			...
			if (
				audibleTabsDetected &&
				(tabAudioSetting === 'pause' || tabAudioSetting === 'reduce')
			) {
				isTabAudible = true;
				if (useServiceWorkerIconPaths) {
					actionApi.setBadgeText({ text: 'll' });
					...
					actionApi
						...
						.then(() => {
							printDebug(
								'[BadgeManager] tabAudio pause event (audible tabs detected), set paused icon with ll text successfully'
							);
						})
						.catch((error) => {
							console.error(
								'[BadgeManager] tabAudio paused icon failed:',
								error
							);
						});
				} else {
					...
					setIcon('paused');
					printDebug(
						'[BadgeManager] tabAudio pause event (audible tabs detected), set paused icon'
					);
				}
			} else {
				isTabAudible = false;
				...
				if (useServiceWorkerIconPaths) {
					...
					actionApi
						...
						.then(() => {
							printDebug(
								`[BadgeManager] tabAudio unpause event (no audible tabs), restored ${lastIconType === 'kk' ? 'KK' : 'weather'} icon:`,
								iconToRestore
							);
						})
						.catch((error) => {
							console.error(
								'[BadgeManager] tabAudio icon restore failed:',
								error
							);
						});
				} else {
					if (lastIconType === 'kk') {
						setIcon('kk');
						printDebug(
							'[BadgeManager] tabAudio unpause event (no audible tabs), restored KK icon'
						);
					} else {
						setIcon(lastWeather);
						printDebug(
							'[BadgeManager] tabAudio unpause event (no audible tabs), set weather icon:',
							lastWeather
						);
					}
				}
